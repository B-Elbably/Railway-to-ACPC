...
struct line {
    ...
    static line bisector(line l1, line l2, bool interior) {
        ...
        pt intersection;
        // FIX: Handle parallel lines fallback
    ...
    bool operator < (const line& l) const { // sort by angle, then distance from origin
        // FIX: Use generic atan2 instead of atan2l to support any type T
...
bool inPolygon(const vector<pt>& p, pt a, bool strict = true) {
    ...
    for (int i = 0, n = p.size(); i < n; i++) {
        ...
        // FIX: Added parentheses around `orient > 0` to fix evaluation order warning
...
void takePoint(pt &p) {
    // FIX: Swapped to T and explicit constructors to avoid narrowing warnings
    T xx, yy; cin >> xx >> yy;
    p = pt(xx, yy);
}

void takeLine(line &l) {
    // FIX: Swapped to T and explicit constructors
    T a, b, c; cin >> a >> b >> c;
    l = line(a, b, c);
}
