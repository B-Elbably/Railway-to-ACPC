...
private:
    ...
    Node query(int x, int lx, int rx, int l, int r) {
        if (lx >= r || rx <= l) return Node(LLONG_MIN); 
...
public:
    SegmentTreemin(const vector<long long> &v) {
        size = 1;
...
private:
    ...
    Node query(int x, int lx, int rx, int l, int r) {
        if (lx >= r || rx <= l) return Node(LLONG_MAX); 
        if (lx >= l && rx <= r) return seg[x];
...
public:
    SegmentTreemax(const vector<long long> &v) {
        size = 1;
        while (size < (int)v.size()) size <<= 1;
        seg.assign(2 * size, Node());
        build(v, 0, 0, size);
...
void solve() {
    ...
    for (auto &x : a) cin >> x;
    SegmentTreemax seg1(a);
    SegmentTreemin seg2(a);
    while (q--) {
        int l, r, w;
        cin >> l >> r >> w;
        int mn = seg1.query(--l, r);
        int mx = seg2.query(l, r);
        cout << ((mn ^ mx ^ w)? "YES" : "NO") << endl;
    }
}
